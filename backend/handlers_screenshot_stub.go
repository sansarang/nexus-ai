...
// ══════════════════════════════════════════════════════════════
//  화면 캡처 + Vision 분석
//  Mac: screencapture 명령 사용
//  결과를 Groq Vision (Llama-4 Scout) 또는 Claude Vision으로 분석
// ══════════════════════════════════════════════════════════════

// captureScreen: 화면 캡처 → 임시 PNG → base64 반환
func captureScreen() (string, error) {
	...
	defer cancel()
	if err := exec.CommandContext(ctx, "screencapture", "-x", "-t", "png", tmp).Run(); err != nil {
		return "", fmt.Errorf("화면 캡처 실패: %w", err)
	}
...
// analyzeImageWithGroqVision: Groq Llama-4 Scout Vision으로 이미지 분석
func analyzeImageWithGroqVision(b64img, question, lang string) (string, error) {
	...
	if json.Unmarshal(raw, &gr) != nil { return "", fmt.Errorf("Vision 응답 파싱 실패") }
	if gr.Error != nil { return "", fmt.Errorf("Vision 오류: %s", gr.Error.Message) }
	if len(gr.Choices) == 0 { return "", fmt.Errorf("Vision 응답 없음") }
	return gr.Choices[0].Message.Content, nil
}

// analyzeImageWithClaude: Claude Vision 분석 (실패 시 Groq fallback)
func analyzeImageWithClaude(b64img, question, lang string) (string, error) {
	...
	if lang == "en" { systemMsg = "You are an AI assistant analyzing screen captures." }
	...
	req.Header.Set("content-type", "application/json")
	resp, err := client.Do(req)
	if err != nil { return analyzeImageWithGroqVision(b64img, question, lang) }
	defer resp.Body.Close()
	raw, _ := io.ReadAll(io.LimitReader(resp.Body, 2*1024*1024))
	var cr struct {
...
func handleScreenshotAnalyze(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if req.Lang == "en" { msg = "Screen capture failed: " + err.Error() }
...
// POST /api/screenshot/translate — 화면 텍스트 번역
