...
%s`, platform, req.Max, contextStr, tr.Summary)
	...
	json200(w, map[string]any{
		...
		"items":   allItems,
		"count":   len(allItems),
		"message": fmt.Sprintf("추천 콘텐츠 %d개", len(allItems)),
	})
}
...
func handleRecallKeywordsMac(w http.ResponseWriter, r *http.Request) {
	...
	json200(w, map[string]any{
		...
		"message":           fmt.Sprintf("최근 %d일 관심 키워드 분석 + 콘텐츠 추천 완료", days),
	})
}
...
func handleContentWishlistAdd(w http.ResponseWriter, r *http.Request) {
	...
	for _, item := range items {
		if item.Title == req.Title {
			json200(w, map[string]any{"success": true, "message": req.Title + "은 이미 위시리스트에 있어요"})
			return
		}
	}
