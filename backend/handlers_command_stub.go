...
// checkMissingParams: action별 필수 파라미터가 없으면 질문 문자열 반환 (없으면 "","false")
func checkMissingParams(action string, params map[string]any, rawMsg string) (string, bool) {
	...
	case "price_compare":
		q := strVal("query")
		// 쇼핑 사이트 키워드를 제거하고 남은 실제 상품명이 짧으면 clarify
		cleanQ := q
		for _, rm := range []string{"쿠팡", "coupang", "11번가", "다나와", "네이버쇼핑", "지마켓", "옥션", "테무", "알리",
			"에서", "찾아줘", "검색해줘", "최저가", "가격", "얼마야", "구매", "사고싶어", "사고 싶어", "추천", "알려줘"} {
			cleanQ = strings.ReplaceAll(cleanQ, rm, "")
		}
		cleanQ = strings.TrimSpace(cleanQ)
		if len([]rune(cleanQ)) < 2 {
			return "어떤 상품을 찾아드릴까요? (예: 에어팟 프로 2, 갤럭시 S25, 다이슨 청소기)", true
		}
		// 예산 키워드가 있는데 금액이 없으면
		if strings.Contains(msgLower, "예산") || strings.Contains(msgLower, "이하") || strings.Contains(msgLower, "이상") {
			hasBudget := false
			for _, ch := range rawMsg {
				if ch >= '0' && ch <= '9' {
					hasBudget = true
					break
				}
			...
			if !hasBudget {
				return "예산이 얼마인지 알려주세요! (예: 30만 원 이하)", true
			}
		}
	...
	case "video_search":
		...
		if q == "" || len([]rune(q)) < 2 {
			...
			if platform == "tiktok" {
				return "TikTok에서 어떤 영상을 찾아드릴까요? (예: 요리, 댄스, 브이로그)", true
			}
			return "YouTube에서 어떤 영상을 찾아드릴까요? (예: 주식 투자 전략, 요리 레시피)", true
		}

	case "trip_plan":
		dest := strVal("destination")
		// 원문+destination 둘 다 합쳐서 도시명 탐색
		searchIn := strings.ToLower(dest) + " " + msgLower
		knownCities := []string{
			"서울", "부산", "제주", "도쿄", "오사카", "삿포로", "후쿠오카", "교토",
			"뉴욕", "LA", "로스앤젤레스", "샌프란시스코", "시카고", "라스베가스",
			"파리", "런던", "암스테르담", "바르셀로나", "로마", "밀라노",
			"방콕", "싱가포르", "홍콩", "대만", "타이베이", "마닐라", "발리", "쿠알라룸푸르",
			"베이징", "상하이", "선전", "청두",
			"두바이", "이스탄불", "시드니", "멜버른",
			"tokyo", "osaka", "new york", "paris", "bangkok", "singapore", "taipei", "dubai",
		}
		foundCity := false
		for _, city := range knownCities {
			if strings.Contains(searchIn, strings.ToLower(city)) {
				foundCity = true
				break
			}
		}
		if !foundCity {
			return "어디로 출장/여행을 가시나요? (예: 도쿄, 뉴욕, 싱가포르, 방콕)", true
		}
		// 날짜 없으면
		date := strVal("date")
		if date == "" {
			hasDate := strings.Contains(rawMsg, "월") || strings.Contains(rawMsg, "일") ||
				strings.Contains(rawMsg, "내일") || strings.Contains(rawMsg, "다음주") ||
				strings.Contains(rawMsg, "다음 주") || strings.Contains(rawMsg, "이번 주") ||
				strings.Contains(rawMsg, "이번주") || strings.Contains(rawMsg, "week")
			if !hasDate {
				return "출발 날짜가 언제인가요? (예: 5월 20일, 다음 주 월요일, 내일)", true
			}
		}

	case "multi_action":
		q := strVal("query")
		subAction := strVal("sub_action")
		if subAction == "summarize" && (q == "" || len([]rune(q)) < 3) {
			return "어떤 주제를 요약·정리해드릴까요?", true
		}
		if subAction == "doc_compare" && (q == "" || len([]rune(q)) < 3) {
			return "무엇과 무엇을 비교해드릴까요? (예: 아이폰16 vs 갤럭시S25)", true
		}

	case "web_search":
		q := strVal("query")
		// 맛집인데 지역 없으면
		isFoodSearch := strings.Contains(msgLower, "맛집") || strings.Contains(msgLower, "식당") || strings.Contains(msgLower, "음식점")
		if isFoodSearch {
			hasLocation := strings.Contains(rawMsg, "구") || strings.Contains(rawMsg, "동") ||
				strings.Contains(rawMsg, "시") || strings.Contains(rawMsg, "역") ||
				strings.Contains(rawMsg, "근처") || strings.Contains(rawMsg, "서울") ||
				strings.Contains(rawMsg, "부산") || strings.Contains(rawMsg, "대구")
			if !hasLocation {
				return "어느 지역 맛집을 찾아드릴까요? (예: 강남, 홍대, 부산 해운대)", true
			}
		}
		// 쿼리가 완전히 비어있으면
		if q == "" || len([]rune(q)) < 2 {
			return "어떤 것을 검색해드릴까요?", true
		}
	}
	return "", false
}
...
func handleCommand(w http.ResponseWriter, r *http.Request) {
	...
	clarifyResp := func(question, pendingAction string, pendingP map[string]any) {
		d := fmt.Sprintf("%.2fs", time.Since(start).Seconds())
		json200(w, CommandResponse{
			Success: true, Message: question, Action: "clarify",
			NeedsClarify: true, ClarifyQuestion: question,
			PendingIntent: pendingAction, PendingParams: pendingP, Duration: d,
		})
	}
	...
	if detectedShopSite != "" && req.PendingIntent == "" {
		...
		// 상품명이 없으면 즉시 clarify
		if len([]rune(q)) < 2 {
			clarifyResp("어떤 상품을 찾아드릴까요? (예: 에어팟 프로 2, 갤럭시 S25, 다이슨 청소기)",
				"price_compare", map[string]any{"site": detectedShopSite})
			return
		}
		if isMultiAction {
		...
		// 검색어 없으면 clarify
		if len([]rune(q)) < 2 {
			clarifyResp("TikTok에서 어떤 영상을 찾아드릴까요? (예: 요리, 댄스, 브이로그, 먹방)",
				"video_search", map[string]any{"platform": "tiktok"})
			return
		}
		if isMultiAction {
			preRoutedAction = "multi_action"
		...
		// 검색어 없으면 clarify
		if len([]rune(q)) < 2 {
			clarifyResp("YouTube에서 어떤 영상을 찾아드릴까요? (예: 주식 투자 전략, 요리 레시피, 운동)",
				"video_search", map[string]any{"platform": "youtube"})
			return
		}
		if isMultiAction {
			preRoutedAction = "multi_action"
	...
	if preRoutedAction == "" && req.PendingIntent == "" {
		...
		knownCitiesList := []string{
			"서울", "부산", "제주", "도쿄", "오사카", "삿포로", "후쿠오카", "교토",
			"뉴욕", "LA", "로스앤젤레스", "샌프란시스코", "시카고", "라스베가스",
			"파리", "런던", "암스테르담", "바르셀로나", "로마", "밀라노",
			"방콕", "싱가포르", "홍콩", "대만", "타이베이", "마닐라", "발리", "쿠알라룸푸르",
			"베이징", "상하이", "선전", "청두", "두바이", "이스탄불", "시드니",
			"tokyo", "osaka", "new york", "paris", "bangkok", "singapore", "taipei", "dubai",
		}
		for _, v := range tripVerbs {
			if strings.Contains(msgLower, v) {
				// 도시명 탐색
				foundDest := ""
				for _, city := range knownCitiesList {
					if strings.Contains(msgLower, strings.ToLower(city)) {
						foundDest = city
						break
					}
				}
				if foundDest == "" {
					clarifyResp("어디로 출장/여행을 가시나요? (예: 도쿄, 뉴욕, 싱가포르, 방콕)",
						"trip_plan", map[string]any{"purpose": "출장"})
					return
				}
