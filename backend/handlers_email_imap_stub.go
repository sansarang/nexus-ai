...
// ══════════════════════════════════════════════════════════════════
//  IMAP/SMTP Email Backend (Mac/Linux) — Windows와 동일한 구현
// ══════════════════════════════════════════════════════════════════
...
func imapAccountsPath() string {
	...
	dir := filepath.Join(configDir, "Nexus")
...
func imapClassifyEmail(subject, body string) imapEmailCategory {
	...
	spamScore, workScore, promoScore := 0, 0, 0
...
func getEmailReplySuggestions(subject, body string) ([]string, error) {
	...
	prompt := fmt.Sprintf(`다음 이메일에 대한 한국어 답장 3가지를 제안해주세요.
...
{"replies": ["답장1", "답장2", "답장3"]}`, subject, body)
	...
	if err != nil {
		return []string{"감사합니다. 확인 후 연락 드리겠습니다.", "네, 알겠습니다.", "좋은 하루 되세요."}, nil
	}
...
func handleIMAPReplySuggestions(w http.ResponseWriter, r *http.Request) {
	...
	json200(w, map[string]any{"success": true, "suggestions": suggestions, "count": len(suggestions)})
}
