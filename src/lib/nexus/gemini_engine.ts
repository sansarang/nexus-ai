...
▶ clarify 해야 할 예시 (실행 자체가 불가능할 때만):
- "파일 찾아줘" (파일명·키워드 전혀 없음) → "파일 이름이나 키워드를 알려주세요"
- "번역해줘" (원문 없음) → "번역할 내용을 알려주세요"
- "이메일 보내줘" (받는 사람 없음) → "받는 사람과 내용을 알려주세요"

▶ clarify 없이 즉시 실행 (추론 실행):
- "날씨 알려줘" → 오늘 한국 날씨 바로 검색
- "뉴스 알려줘" → 오늘 주요 뉴스 바로 검색
- "주식 알려줘" → 코스피/코스닥 현황 바로 검색
- "버스 시간표" → 질문 그대로 검색
- 부분 정보가 있으면 추론해서 즉시 실행 → 결과와 함께 "더 구체적으로 원하시면 말씀해 주세요" 안내

핵심 판단 기준: 실행에 필요한 최소 정보(검색어·대상·내용)가 하나라도 있으면 → 즉시 실행. 아무 힌트도 없어서 어떤 액션도 불가능할 때만 → clarify.
...
export async function callOllama(
  ...
  const ollamaModel = localStorage.getItem('nexus-ollama-model') ?? 'llama3.2'
...
export async function callGemini(
  ...
  userInput: string,
  history: ConversationTurn[],
  isPro = false, // Pro 구독자만 GPT-4o 사용 (비용 절감)
): Promise<GeminiResponse> {
  // Pro 구독자만 GPT-4o 사용
  if (isPro) {
    const openaiKey = OPENAI_API_KEY || localStorage.getItem('nexus-openai-key') || ''
    if (openaiKey) {
      try { return await callGPT4oWithTools(openaiKey, userInput, history) } catch { /* Perplexity로 폴백 */ }
    }
  }
  // 무료 사용자 또는 GPT-4o 실패 → Perplexity sonar-pro
...
export async function callGroq(
  ...
  if (!res.ok) {
    ...
    // 429 Rate limit → 사용자 안내 (다른 유료 API 자동 전환 금지)
    if (res.status === 429) {
      return {
        text: '잠시 요청이 몰려 응답이 지연되고 있어요. 10~20초 후 다시 말씀해주세요. 🙏',
