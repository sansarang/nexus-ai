...
export type Intent =
  ...
  | 'clipboard_history' // 클립보드 히스토리 보기/삭제
  // ── 📝 음성 메모→할일 ───────────────────────────
...
const PATTERNS: { intent: Intent; patterns: RegExp[] }[] = [
  ...
  // ── 📅 자연어 일정 추가 (날짜·시간 포함) — calendar_add보다 먼저 체크 ──
  {
    intent: 'calendar_smart_add',
    patterns: [
      /다음.*주.*미팅|내일.*점심.*일정|오후.*회의.*추가|스마트.*일정/i,
      /(".*")\s*(일정|미팅|약속).*추가|일정.*자연어.*추가/i,
      // 날짜·시간 정보 포함된 자연어 일정 등록
      /(내일|모레|다음\s*주|이번\s*주|\d+월\s*\d+일|\d+일|\d+시).*(일정|미팅|회의|약속).*(추가|등록|잡아|넣어)/i,
      /(일정|미팅|회의|약속).*(추가|등록|잡아|넣어).*(내일|모레|다음\s*주|\d+월|\d+일|\d+시)/i,
    ],
  },
  // ── 📅 캘린더: 단순 일정 추가 (날짜 없음) ──
  {
  ...
  // ── 📋 클립보드 히스토리 ──
  {
    ...
    patterns: [
      /클립보드.*히스토리|클립보드.*기록|복사.*기록|클립보드.*목록/i,
      /clipboard.*history|clipboard.*log|past.*copies/i,
    ],
  },
